...
import { getStaffProfileStats, getMyServices } from "@/actions/staffActions";
    ...
    if (!session.isAuth) {
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-lg">Access denied. <Link href="/staff/login" className="text-blue-600">Login</Link></div>
            </div>
        );
    }
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                <span className="text-yellow-500">
                                    ...
                                    <span className="text-gray-600 ml-1">({stats?.rating || 0})</span>
                                </span>
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-green-50 p-4 rounded text-center">
                        <div className="text-2xl font-bold text-green-600">{stats?.successfulServices || 0}</div>
                        <div className="text-sm text-gray-600">Completed</div>
                    </div>
                    <div className="bg-red-50 p-4 rounded text-center">
                        <div className="text-2xl font-bold text-red-600">{stats?.canceledServices || 0}</div>
                        <div className="text-sm text-gray-600">Canceled</div>
                    </div>
                    <div className="bg-blue-50 p-4 rounded text-center">
                        <div className="text-2xl font-bold text-blue-600">{staffData.hasRepairExperience && staffData.repairExperienceYears ||
                            (staffData.role === 'technician'
                                ? staffData.repairExperienceYears || 0
                                : staffData.installationExperienceYears || 0)}</div>
                ...
                <div className="space-y-4">
                    <div><strong>Phone:</strong> {staffData.phone}</div>
                    <div><strong>Address:</strong> {staffData.currentStreetAddress}, {staffData.currentDistrict}, {staffData.currentPoliceStation}, {staffData.currentPostOffice}</div>
                    <div><strong>Skills:</strong> {typeof staffData.skills === 'string'
                        ? (() => {
                            try {
                                return (JSON.parse(staffData.skills || '[]') as string[]).join(', ');
                            } catch {
                                return staffData.skills || 'N/A';
                            }
                        })()
                        : (staffData.skills || []).join(', ')}</div>
                    <div><strong>Bio:</strong> {staffData.bio || 'N/A'}</div>
                    <div><strong>Father's Name:</strong> {staffData.fatherName}</div>
                    <div><strong>Permanent Address:</strong> {staffData.permanentStreetAddress}, {staffData.permanentDistrict}, {staffData.permanentPoliceStation}, {staffData.permanentPostOffice}</div>
                    <div><strong>Payment Preference:</strong> {staffData.paymentPreference}</div>
                    {staffData.walletNumber && <div><strong>Wallet Number:</strong> {staffData.walletNumber}</div>}
                                    ...
                                    {(() => {
                                        try {
                                            ...
                                            return docs.map((doc: string, idx: number) => (
                                                <a key={idx} href={doc} target="_blank" rel="noopener noreferrer" className="bg-gray-100 px-3 py-1 rounded text-sm text-blue-600 hover:underline">
                                                    Doc {idx + 1}
                                                </a>
                                            ));
                                        } catch {
                            ...
                            <tbody>
                                {services.map((service: any) => (
                                    <tr key={service.serviceId} className="border-b">
                                        <td className="p-2 font-mono text-sm">{service.serviceId}</td>
                                        <td className="p-2">{service.customerName}</td>
                                        <td className="p-2">{service.productModel} ({service.productType})</td>
                                        <td className="p-2">{service.type}</td>
                                        <td className="p-2">
                                            <span className={`px-2 py-1 rounded text-xs ${getStatusColor(service.statusHistory?.[0]?.status || 'pending')}`}>
                                                {service.statusHistory?.[0]?.status || 'pending'}
                                            </span>
                                        </td>
                                        <td className="p-2">{new Date(service.createdAt).toLocaleDateString()}</td>
                                        <td className="p-2">
                                            {service.statusHistory?.[0]?.status !== 'completed' && service.statusHistory?.[0]?.status !== 'canceled' && (
                                                <Link
