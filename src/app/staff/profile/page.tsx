...
export default async function StaffProfilePage() {
  ...
  const activeComplaints = complaintsRes.success ? (complaintsRes.data || []) : [];
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50 p-2">
        <div className="w-full max-w-sm bg-white rounded-3xl p-8 shadow-xl border border-rose-100 text-center flex flex-col items-center gap-6 animate-in zoom-in duration-300">
           ...
           <div>
              <h1 className="text-2xl font-black text-gray-900 mb-2 underline decoration-rose-500 underline-offset-4">ACCOUNT BLOCKED</h1>
              <p className="text-gray-500 text-sm font-medium">Your access to the portal has been suspended by the administration.</p>
           </div>
           
           <div className="w-full p-5 bg-gray-50 rounded-2xl border border-gray-100 space-y-3 text-left">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest text-center">Next Steps</p>
              <p className="text-sm text-gray-700 font-bold">Please contact the head office or branch manager to re-activate your account.</p>
              <div className="flex justify-center">
                <a href={`tel:${adminPhone}`} className="inline-flex items-center gap-2 text-brand font-black text-base hover:underline mt-2">
                  Call Administration
                </a>
              </div>
           </div>
