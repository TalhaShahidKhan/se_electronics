    ...
    <StaffLayout balance={stats?.availableBalance || 0}>
      <div className="p-4 space-y-6">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Customer Feedbacks</h1>
        </div>
        ...
        {feedbacksList.length === 0 ? (
          <div className="bg-white p-12 rounded-2xl shadow-sm border border-gray-100 text-center text-gray-500">
            ...
            <p className="text-sm mt-1 text-gray-400 font-medium">When customers give feedback on your services, they&apos;ll appear here.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {feedbacksList.map((feedback: any) => (
              <div key={feedback.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-lg font-bold text-gray-900 truncate">{feedback.customerName}</h3>
                    <p className="text-xs font-semibold text-gray-500 mt-1 uppercase tracking-tight">
                      {feedback.productModel} • {feedback.productType}
                    </p>
                    <p className="text-[10px] font-bold font-mono text-gray-300 mt-1 uppercase">Order #{feedback.serviceId.substring(0, 8)}</p>
                  </div>
                    ...
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star 
                          key={s} 
                          size={14} 
                          className={s <= (feedback.rating || 0) ? "text-yellow-400 fill-yellow-400" : "text-gray-200"} 
                        />
                      ))}
                    </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 font-bold uppercase">Submitted On</span>
                    <span className="text-xs text-gray-600 font-bold">
                      {new Date(feedback.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 bg-brand/5 px-3 py-1 rounded-full border border-brand/10">
                    <span className="text-[10px] font-bold text-brand uppercase tracking-tight">Verified</span>
