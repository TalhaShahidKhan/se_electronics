    ...
    if (!session.isAuth || !session.customer) {
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-center">
                 <div>
                    <h2 className="text-2xl font-bold mb-4">Please log in to view your complaints</h2>
                    <Link href="/customer/login" className="bg-brand text-white font-bold py-3 px-6 rounded-lg">Login</Link>
                </div>
            </div>
        );
    }
                ...
                <div className="mb-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                            My Complaints
                            <span className="text-xs font-bold bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full uppercase tracking-widest">{complaints.length}</span>
                        </h1>
                    </div>
                    <Link href="/customer/complain" className="bg-brand text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-md shadow-brand/20 hover:scale-105 transition-all">
                        File New Complaint
                    </Link>
                </div>
                ...
                <div className="space-y-4">
                    {complaints.length === 0 ? (
                        <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 mb-1">No complaints filed</h3>
                            <p className="text-gray-500 text-sm">When you file a complaint about staff, it will appear here.</p>
                        </div>
                    ) : (
                        complaints.map((c: any) => (
                            <div key={c.id} className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 group hover:shadow-md hover:border-brand/20 transition-all">
                                <div className="flex-1 space-y-2">
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 font-medium">
                                        ...
                                        <span className="flex items-center gap-1">
                                            ...
                                            Against: <span className="text-gray-900 font-bold">{c.staff?.name}</span>
                                        </span>
                                    </div>
                                </div>
