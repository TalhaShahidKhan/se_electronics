import { verifyCustomerSession, applyForVipCard } from "@/actions/customerActions";
                    ...
                    <div className="bg-[#1d4ed8] rounded-[2rem] p-8 sm:p-12 text-white shadow-2xl relative overflow-hidden group border-4 border-white/20">
                        ...
                        <div className="relative z-10 font-mono">
                            <div className="flex justify-between items-start mb-12">
                                <div>
                                    <p className="text-blue-100 font-bold uppercase tracking-[0.2em] text-xs mb-2">SE Electronics</p>
                                    <h2 className="text-3xl font-black tracking-tight text-white/90">VIP PREMIER</h2>
                                </div>
                                <Crown size={40} className="text-white opacity-80" />
                            </div>

                            <div className="mb-10 font-mono">
                                <p className="text-blue-100/60 text-[10px] uppercase tracking-[0.3em] mb-3">VIP Card Number</p>
                                <p className="text-2xl sm:text-3xl font-bold tracking-[0.2em] text-white">
                                    {vipCardNumber ? vipCardNumber.match(/.{1,4}/g)?.join(' ') : "#### #### #### ####"}
                                </p>
                            </div>
                            
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-blue-100/60 text-[10px] uppercase tracking-[0.3em] mb-1">Cardholder</p>
                                    <p className="text-lg font-bold uppercase tracking-wider">{customer.name}</p>
                                </div>
                    ...
                    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-[2rem] p-8 sm:p-12 text-gray-800 shadow-sm border border-gray-200 relative overflow-hidden group">
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-8">
                                <div>
                                    <p className="text-gray-400 font-bold uppercase tracking-[0.2em] text-xs mb-2">SE Electronics</p>
                                    <h2 className="text-2xl font-black tracking-tight text-gray-900">VIP Card Application</h2>
                                </div>
                ...
                <div className="mt-12 bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                    ...
                    <ul className="space-y-5 mb-8">
                        {benefits.map((benefit, i) => (
                            <li key={i} className="flex items-start gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                                <div className="mt-1 bg-blue-100 rounded-full p-1.5 border border-blue-200">
                                    <CheckCircle2 size={16} className="text-blue-600" />
                                </div>
                                <span className="text-gray-700 leading-snug font-medium">{benefit}</span>
                            </li>
                        ))}
                    </ul>
                    ...
                    <div className="bg-gray-900 rounded-3xl p-8 text-white relative overflow-hidden">
                        ...
                        <p className="text-sm text-gray-400 mb-8 leading-relaxed font-medium">
                            If you have any questions about our VIP program or your application, our support team is ready to help.
                        </p>
                        <a 
                            href={`https://wa.me/${contactDetails.whatsApp.replace(/\+/g, '')}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center justify-center gap-3 w-full bg-[#25D366] hover:bg-[#128C7E] text-white font-black py-4 px-6 rounded-2xl shadow-lg transition-all active:scale-95 text-xs uppercase tracking-widest"
                        >
