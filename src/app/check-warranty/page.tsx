...
export default function CheckWarrantyPage() {
    ...
    return (
        <div className="mx-auto max-w-[1200px] min-h-screen p-3 flex flex-col gap-4" style={{ backgroundColor: "rgb(239, 246, 255)" }}>
            {/* Header */}
            <div className="font-bold flex flex-col gap-0.5 p-6 rounded-lg border-2 text-center bg-white">
                <div className="text-lg">এস ই ইলেকট্রনিকস প্রডাক্ট ওয়ারেন্টি ট্রেকিং</div>
            ...
            {warrantyData && (
                <div className="space-y-3 mb-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="font-semibold text-gray-800">{warrantyData.customerName}</div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1">
                                    <div className="text-xs text-gray-500 mb-1">কাস্টমার আইডি</div>
                                    <div className="font-semibold text-gray-800">{warrantyData.customerId}</div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="font-semibold text-gray-800">{warrantyData.customerPhone}</div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1">
                                    <div className="text-xs text-gray-500 mb-1">ক্রয়ের তারিখ</div>
                                    <div className="font-semibold text-gray-800">{formatDate(warrantyData.date)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border-2 rounded-lg p-4 bg-white">
                        <h3 className="text-xl font-bold mb-4 border-b pb-2 text-gray-800">Purchased Products</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {warrantyData.products.map((product, index) => {
                                const isValid = isWarrantyValid(product.warrantyStartDate, product.warrantyDurationMonths);
                                const expireDate = calculateWarrantyEndDate(product.warrantyStartDate, product.warrantyDurationMonths);
                                ...
                                return (
                                    <div key={index} className="p-5 border rounded-xl">
                                        ...
                                        {/* Status Tag */}
                                        <div className="mb-3">
                                            {product.warrantyDurationMonths > 0 ? (isValid ? (
                                        ...
                                        {/* Product Details */}
                                        <h4 className="text-lg font-bold text-gray-900 mb-2">{product.type.toUpperCase()}</h4>

                                        <div className="space-y-3">
                                            <div className="flex items-center text-gray-600">
                                                <div className="text-sm font-medium">Model: <span className="font-semibold text-gray-800">{product.model}</span></div>
                                            </div>
                                            <div className="flex items-center text-gray-600">
                                                {product.warrantyDurationMonths === 0 ? (
                                                    <div className="text-sm font-medium">Warranty: <span className=" text-gray-400">N/A</span></div>
                                                ) : (
                                                    <div className="text-sm font-medium">Warranty: <span className="font-semibold text-gray-800">{product.warrantyDurationMonths} Months</span></div>
                                                )}
                                            </div>
                                            <div className="flex items-center text-gray-600">
                                                <div className="text-sm font-medium">Warranty Start Date: <span className="font-semibold text-gray-800">{formatDate(product.warrantyStartDate)}</span></div>
                                            </div>
                                            {product.warrantyDurationMonths > 0 &&
                                                <div className="flex flex-col items-start text-gray-600 pt-2 border-t mt-3">
                                                    <div className="text-xs text-gray-500 mb-0.5">{isValid ? 'ওয়ারেন্টি শেষ হবে:' : 'ওয়ারেন্টি শেষ হয়েছে:'}</div>
                                                    <div className={`font-bold text-red-600`}>{formatDate(expireDate)}</div>
                                                </div>
                                            }
                                        </div>
                                    </div>
