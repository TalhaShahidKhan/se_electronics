...
export default async function generatePDF(args: {
  docType: DocType;
  id: string;
  token: string;
}): ReturnType {
  try {
    ...
    if (args.token) {
      ...
      if (!session) {
        throw new AppError(
          "এই ফাইলটি ডাউনলোড করার জন্য আপনার প্রয়োজনীয় অনুমতি নেই।",
        );
      }
    ...
    switch (docType) {
      case "invoice": {
        ...
        const bgImageBase64 = await convertToBase64(
          path.join(
            process.cwd(),
            "src",
            "assets",
            "images",
            response.data!.dueAmount > 0
      ...
      case "id-card": {
        ...
        const frontBgImageBase64 = await convertToBase64(
          path.join(
            process.cwd(),
            "src",
            "assets",
            "images",
            response.data!.role === "electrician"
              ? "electrician-card.jpg"
              : "technician-card.jpg",
          ),
        );
        const backBgImageBase64 = await convertToBase64(
          path.join(
            process.cwd(),
            "src",
            "assets",
        ...
        const qrcodeBase64 = await QRCode.toDataURL(baseUrl + "/team-members", {
          width: 80,
          margin: 2,
      ...
      case "certificate": {
        ...
        const staffProfileUrl =
          baseUrl + "/team-members?staffId=" + certificateData!.staffId;
        ...
        const [
          ...
          QRCode.toDataURL(staffProfileUrl, {
            width: 100,
            margin: 2,
            color: { light: "#00000000" },
          }),
      ...
      case "complaint": {
        ...
        const data = {
          ...
          staff: {
            ...complaint.staff,
            currentStreetAddress: complaint.staff.currentStreetAddress,
            currentDistrict: complaint.staff.currentDistrict,
          },
          productType:
            productMap[complaint.service?.productType || ""] || "ইলেকট্রনিক্স",
      ...
      case "hearing-notice": {
        ...
        html = `
                <div style="font-family: 'SolaimanLipi', serif;  max-width: 820px; margin: 0 auto; background: #fff;  font-size: 15px; color: #111;">
                  ...
                  <div style="margin-bottom: 24px;">
                    ...
                    <p><strong>টেকনিশিয়ান নামঃ</strong> ${c.staff?.name}<strong>টেকনিশিয়ান আইডিঃ</strong> ${c.staffId}<strong>ভূমিকাঃ</strong> ${c.staff?.role?.toUpperCase()}<strong>ফোন নম্বরঃ</strong> ${c.staff?.phone}</p>
                  </div>

                  <div style="margin-bottom: 28px;">
                    <p style="font-weight: 700; margin-bottom: 8px;">শুনানির বিস্তারিত নোটিশঃ</p>
                    <p style="text-align: justify; background: #fff; border-left: 4px solid #d97706; padding: 14px 18px; line-height: 1.9;">
