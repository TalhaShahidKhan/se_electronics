...
import { applications, products, services, serviceStatusHistory } from "@/db/schema";
import { sendEmail, sendSMS, SMSError } from "@/lib";
...
import { AddToServiceSchema, AppointmentDataSchema, ServiceDataSchema, ServiceReportDataSchema, UpdateServiceDataSchema } from "@/validationSchemas";
...
export const getServices = async ({ query, page = '1', limit = '20', type }: SearchParams & { type: 'repair' | 'install' }) => {
    try {
        const q = `%${query}%`
...
export const getServicesMetadata = async ({ query, page = '1', limit = '20', type }: SearchParams & { type: 'repair' | 'install' }) => {
    const q = `%${query}%`
    const filters = and(
        eq(services.isActive, true),
        eq(services.type, type),
...
export const getServiceById = async (serviceId: string) => {
    try {
        const serviceData = await db.query.services.findFirst({
            ...
            with: {
                statusHistory: {
                    ...
                    orderBy: (statusHistory, { desc, asc }) => [asc(statusHistory.createdAt)]
                },
                appointedStaff: {
                    columns: {
                        staffId: true,
        ...
        if (serviceData.appointedStaff) {
            const staffPhotoUrl = await getObjectUrl(serviceData.appointedStaff.photoKey)
            return { success: true, data: { ...serviceData, appointedStaff: { ...serviceData.appointedStaff, photoUrl: staffPhotoUrl } } }
        } else {
...
export async function createService(prevState: any, formData: FormData) {
    try {
        console.log("createService called with formData keys:", Array.from(formData.keys()));
        ...
        const { productFrontPhoto, productBackPhoto, warrantyCardPhoto, ...serviceData } = validatedCustomerData
        ...
        if (includesMedia) {
            ...
            if (!mimeTypes.includes(validatedCustomerData.productFrontPhoto!.type) ||
        ...
        await db.transaction(async (tx) => {
            if (includesMedia) {
                ...
                const [
                    ...
                    compressImage(Buffer.from(await productFrontPhoto.arrayBuffer()), 'product'),
                    compressImage(Buffer.from(await productBackPhoto.arrayBuffer()), 'product'),
                    compressImage(Buffer.from(await warrantyCardPhoto.arrayBuffer()), 'warranty'),
                ])
            ...
            await tx.insert(services).values({
                ...
                isActive: originSource === 'dashboard' ? true : false,
                ipAddress: ipAddress,
                userAgent: userAgent,
                productBackPhotoKey,
        ...
        if (originSource === 'public_form') {
            ...
            await Promise.all([
                sendSMS(
                    ...
                    renderText(
                        ApplicationMessages.service.SUBMISSION,
                        {
            ...
            const smsMessageContent = renderText(
        ...
        if (error instanceof z.ZodError) {
            console.error("Zod Validation Errors:", z.flattenError(error).fieldErrors);
...
export const appointStaff = async (appointmentData: z.infer<typeof AppointmentDataSchema>) => {
    try {
            ...
            sendSMS(
                ...
                renderText(
                        ...
                        ServiceMessages.TECHNICIAN_APPOINT,
                    {
                        ...
                        service_report_url: generateUrl('service-report', { serviceId: validatedData.serviceId })
                    }
                )
            )
        ])
...
export const updateService = async (formData: FormData, serviceId: string, statusId?: string) => {
    try {
        const validatedData = UpdateServiceDataSchema.parse(Object.fromEntries(formData))
        const {
            serviceStatus,
            customLabel,
            customNote,
            ...
            productFrontPhoto,
            productBackPhoto,
            ...restData } = validatedData

        const currentServiceStatus = (await db.query.serviceStatusHistory.findFirst({
        ...
        if (serviceStatus === 'pending') {
            await db.delete(serviceStatusHistory).where(eq(serviceStatusHistory.serviceId, serviceId))
        } else {
            await db.delete(serviceStatusHistory).where(
                and(
        ...
        switch (serviceStatus) {
            ...
            case 'custom': {
                await db.update(serviceStatusHistory)
                    .set({
                        customLabel: customLabel,
                        customNote: customNote
                    })
            ...
            default: {
                await db.insert(serviceStatusHistory).values({
                    ...
                    ...(serviceStatus === 'canceled' && cancelReason ? { cancelReason: cancelReason } : {})
                })
                break
            }
        }
        ...
        if (warrantyCardPhoto && mimeTypes.includes(warrantyCardPhoto.type) && warrantyCardPhoto.size > 0) {
            const warrantyCardPhotoBuffer = Buffer.from(await warrantyCardPhoto.arrayBuffer());
            promisesArray.push(
                putObject({
        ...
        if (productFrontPhoto && mimeTypes.includes(productFrontPhoto.type) && productFrontPhoto.size > 0) {
            const productFrontPhotoBuffer = Buffer.from(await productFrontPhoto.arrayBuffer());
            promisesArray.push(
                putObject({
        ...
        if (productBackPhoto && mimeTypes.includes(productBackPhoto.type) && productBackPhoto.size > 0) {
            const productBackPhotoBuffer = Buffer.from(await productBackPhoto.arrayBuffer());
            promisesArray.push(
                putObject({
...
export const reportService = async ({ serviceStatus, serviceReport, messageData }: z.infer<typeof ServiceReportDataSchema>) => {
    try {
        ...
        if (messageData) {
            const messageContent = renderText(
                ...
                ),
                {
                    ...
                    feedback_url: generateUrl('feedback', { serviceId: serviceStatus.serviceId }),
                }
            )
...
export async function deleteService(serviceId: string) {
    try {
        await db.transaction(async (tx) => {
            const serviceData = await tx.delete(services)
                .where(eq(services.serviceId, serviceId))
                .returning({
            ...
            await tx.delete(applications).where(eq(applications.applicantId, serviceId))
