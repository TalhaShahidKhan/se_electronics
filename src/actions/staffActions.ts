...
import { ApplicationMessages, MediaDownloadMessages } from "@/constants/messages";
import { db } from "@/db/drizzle";
import { agreements, applications, authTokens, feedbacks, payments, services, staffs, userAgreements } from "@/db/schema";
import { createSession, decrypt, deleteSession, sendSMS, SMSError } from "@/lib";
import { deleteObject, getObjectUrl, putObject } from "@/lib/s3";
import { compressImage } from "@/lib/sharp";
import { CertificateData, FormState, SearchParams } from "@/types";
import { generateRandomId, generateUrl, renderText } from "@/utils";
import { LoginCredentialsSchema, StaffDataSchema, UpdateStaffDataSchema } from "@/validationSchemas";
...
export const sendRegistrationLink = async (phoneNumber: string) => {
    try {
        ...
        const expiresAt = new Date(Date.now() + parseInt(process.env.REGISTRATION_LINK_EXPIRY_DAY!) * 24 * 60 * 60 * 1000)
        ...
        const registrationMessage = renderText(
            ApplicationMessages.staff.REG_INVITE,
            {
                ...
                registration_link_expiry: (parseInt(process.env.REGISTRATION_LINK_EXPIRY_DAY!) * 24).toString()
            }
        )
...
export const sendIdCardDownloadLink = async (staffData: { phoneNumber: string, staffId: string, staffName: string, role: 'technician' | 'electrician' }) => {
    try {
        ...
        const message = renderText(
            role === 'electrician' ? MediaDownloadMessages.ELECTRICIAN_ID_CARD : MediaDownloadMessages.TECHNICIAN_ID_CARD,
            {
...
export const sendCertificateLink = async (formData: FormData) => {
    try {
        ...
        const message = renderText(
            MediaDownloadMessages.CERTIFICATE_DOWNLOAD,
            {
...
export const getAllTeamMembers = async () => {
    try {
        ...
        const finalStaffData = await Promise.all(staffsData.map(async (staff) => {
            const photoUrl = await getObjectUrl(staff.photoKey)
            return {
                ...staff,
                photoUrl
            }
...
export const getStaffs = async ({ query, page = '1', limit = '20', role }: SearchParams & { role?: 'technician' | 'electrician' }) => {
    try {
        const q = `%${query}%`
...
export const getStaffsMetadata = async ({ query, page = '1', limit = '20', role }: SearchParams & { role?: 'technician' | 'electrician' }) => {
    const q = `%${query}%`
    const filters = and(
        eq(staffs.isVerified, true),
        role && eq(staffs.role, role),
...
export const getStaffById = async (staffId: string) => {
    try {
        ...
        const photoUrl = await getObjectUrl(staffData.photoKey)

        return { success: true, data: { ...staffData, photoUrl } }
    } catch (error) {
        console.error(error)
    ...
    try {
        ...
        await db.transaction(async (tx) => {
            ...
            await tx.insert(staffs).values({
                ...
                nidFrontPhotoKey,
                nidBackPhotoKey,
                docs: validatedStaffData.data.docs ? JSON.stringify(validatedStaffData.data.docs) : '[]',
                role: staffData.hasInstallationExperience ? 'electrician' : 'technician',
        ...
        if (sendConfirmationSMS) {
            await sendSMS(
                staffData.phone,
                renderText(
                    ApplicationMessages.staff.APPROVAL,
                    {
        ...
        if (originSource === 'public_form' && applicationId) {
            await sendSMS(
                staffData.phone,
                renderText(
                    ApplicationMessages.staff.SUBMISSION,
                    {
                        applicant_name: staffData.name,
        ...
        return { success: true, message: 'Added successfully', data: { name: staffData.name } }
...
export const updateStaff = async (staffId: string, data: FormData) => {
    try {
        ...
        const staffData = await db.update(staffs)
            .set({
                ...restStaffData,
                docs: validatedStaffData.docs ? JSON.stringify(validatedStaffData.docs) : undefined,
                role: restStaffData.hasInstallationExperience ? 'electrician' : 'technician'
            })
            .where(eq(staffs.staffId, staffId))
            .returning({
        ...
        if (nidFrontPhoto) {
            const nidFrontPhotoBuffer = Buffer.from(await nidFrontPhoto.arrayBuffer())
            promisesArray.push(putObject({
        ...
        console.error(error)
        return { success: false, message: 'Could not update staff. Something went wrong' }
    }
}
...
export const deleteStaff = async (staffId: string) => {
    try {
        await db.transaction(async tx => {
            const serviceData = await tx.delete(staffs)
                .where(eq(staffs.staffId, staffId))
                .returning({
                    photoKey: staffs.photoKey,
                    nidFrontPhotoKey: staffs.nidFrontPhotoKey,
            ...
            await tx.delete(applications).where(eq(applications.applicantId, staffId))
...
export async function verifyStaffSession() {
    ...
    const staff = await db.query.staffs.findFirst({
        where: eq(staffs.staffId, session.userId),
        columns: {
            id: true,
            staffId: true,
            name: true,
    ...
    return { isAuth: true, userId: session.userId, username: session.username, role: 'staff', staff }
}
...
export async function updateMyProfile(staffId: string, data: FormData) {
    try {
        ...
        if (profileData.currentStreetAddress) staffData.currentStreetAddress = profileData.currentStreetAddress;
        if (profileData.currentDistrict) staffData.currentDistrict = profileData.currentDistrict;
        if (profileData.currentPoliceStation) staffData.currentPoliceStation = profileData.currentPoliceStation;
        if (profileData.currentPostOffice) staffData.currentPostOffice = profileData.currentPostOffice;
        if (skills && skills.toString().trim()) {
            const skillsArray = skills.toString()
                .split(',')
                .map(s => s.trim())
                .filter(s => s.length > 0);
...
export async function setStaffCredentials(staffId: string, username: string, password: string) {
    try {
...
export async function getStaffProfileStats(staffId: string) {
    try {
        ...
        const totalServices = await db.$count(services, eq(services.staffId, staffId));

        // Get successful (completed) services
        const successfulServices = await db.$count(services, and(
            eq(services.staffId, staffId),
            eq(services.resolvedBy, 'staff_member')
        ));

        // Get canceled services
        const canceledServices = await db.$count(services, and(
            eq(services.staffId, staffId),
...
export async function updateStaffStats(staffId: string) {
    try {
        await db.transaction(async (tx) => {
            ...
            const totalCount = await tx.$count(services, eq(services.staffId, staffId));
            const successCount = await tx.$count(services, and(
                eq(services.staffId, staffId),
                eq(services.resolvedBy, 'staff_member')
            ));
            const canceledCount = await tx.$count(services, and(
                eq(services.staffId, staffId),
