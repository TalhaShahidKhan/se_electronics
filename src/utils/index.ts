...
export const generateUrl = <T extends UrlConfig['type']>(type: T, params: Extract<UrlConfig, { type: T }>['params']) => {
    ...
    switch (type) {
        case 'registration':
            ...
            return `${baseUrl}/register?token=${params.token}`

        case 'application-tracking':
            if (!params.trackingId) {
                throw new Error('trackingId is required')
            }
            return `${baseUrl}/application-track?trackingId=${params.trackingId}`

        case 'service-tracking':
            if (!params.trackingId) {
                throw new Error('trackingId is required')
            }
            return `${baseUrl}/service-track?trackingId=${params.trackingId}`

        case 'service-report':
            if (!params.serviceId) {
                throw new Error('serviceId is required')
            }
            return `${baseUrl}/service-report?serviceId=${params.serviceId}`

        case 'invoice-download':
            if (!params.token) {
                throw new Error('token is required')
            }
            return `${baseUrl}/pdf/download?token=${params.token}`
        ...
        case 'feedback':
            ...
            return `${baseUrl}/service-feedback?serviceId=${params.serviceId}`
...
export const formatDate = (timestamp: Date | string | number, includeTime = false) => {
...
export function isWarrantyValid(warrantyStartDate: Date, warrantyDurationMonths: number): boolean {
...
export function calculateWarrantyEndDate(startDate: Date, durationMonths: number): Date {
