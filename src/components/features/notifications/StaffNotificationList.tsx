...
import { CombinedNotificationType, StaffNotificationType, NoticeRecipientType } from "@/types";
import { useState, useEffect } from "react";
import { Bell, CheckCircle2, ChevronRight, Info, AlertTriangle, Zap, Inbox, Wallet, Wrench } from "lucide-react";
...
export default function StaffNotificationList() {
  ...
  const fetchData = async () => {
    ...
    if (actionNotifRes.success && actionNotifRes.data) {
      combined = [...combined, ...(actionNotifRes.data as StaffNotificationType[]).map(n => ({ ...n, itemType: 'action' as const }))];
    }

    if (noticeNotifRes.success && noticeNotifRes.data) {
      combined = [...combined, ...(noticeNotifRes.data as NoticeRecipientType[]).map(n => ({ ...n, itemType: 'notice' as const }))];
    }
      ...
      <div className="grid grid-cols-1 gap-3">
        {notifications.map((item) => {
          ...
          if (item.itemType === 'action') {
            title = item.type === 'balance_added' ? 'Balance Added' : 'Notification';
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                item.isRead ? "border-gray-50 opacity-80" : "border-brand/20 shadow-md ring-2 ring-brand/5"
              )}
            >
              <div className={clsx(
                "shrink-0 size-12 lg:size-14 rounded-xl lg:rounded-2xl flex items-center justify-center text-white shadow-lg",
                item.itemType === 'action' ? (item.type === 'balance_added' ? 'bg-emerald-500' : 'bg-brand') : (
                  item.notice?.priority === "urgent" ? "bg-rose-500" : 
                  item.notice?.priority === "high" ? "bg-orange-500" : "bg-blue-500"
                )
              )}>
                {item.itemType === 'action' ? (
                  item.type === 'balance_added' ? <Wallet size={20} /> : <Info size={20} />
                ) : (
                  item.notice?.priority === "urgent" ? <Zap size={20} /> : <Info size={20} />
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs lg:text-sm text-gray-500 line-clamp-1 font-medium leading-relaxed">
                  {message}
                </p>
