...
export default function NoticeBanner({ notifications }: { notifications: NoticeRecipientType[] }) {
  const [currentNotice, setCurrentNotice] = useState<NoticeRecipientType | null>(null);

  useEffect(() => {
    // Find the most recent unread urgent/high priority notice
    const importantUnread = notifications
      .filter(n => !n.isRead && (n.notice?.priority === "urgent" || n.notice?.priority === "high"))
  ...
  return (
    <div className={clsx(
      ...
      currentNotice.notice?.priority === "urgent" ? "bg-rose-600 text-white" : "bg-orange-500 text-white"
    )}>
      <div className="flex items-center gap-4 min-w-0">
        <div className="size-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
          {currentNotice.notice?.priority === "urgent" ? <Zap size={20} /> : <AlertTriangle size={20} />}
        </div>
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.2em] opacity-70 mb-0.5">
            {currentNotice.notice?.priority} Announcement
          </p>
          <h4 className="text-sm font-bold truncate">{currentNotice.notice?.title}</h4>
        </div>
      </div>
