...
import { Plus, Search, Trash2, Edit3, Eye, MoreVertical, Bell, Info, AlertTriangle, Zap, CheckCircle2, UserCheck, Calendar } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 py-3.5 bg-brand text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-brand-800 transition-all shadow-lg shadow-brand/20 flex items-center justify-center gap-2"
        >
          <Plus size={18} />
          New Notice
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNotices.map((notice) => (
          <div key={notice.id} className="group bg-white rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl hover:border-brand/20 transition-all overflow-hidden flex flex-col">
            {/* Header */}
            <div className="p-6 pb-0 flex justify-between items-start">
              <div className={clsx(
                "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                {
                  ...
                  "bg-emerald-50 text-emerald-600": notice.priority === "normal",
               ...
               <div className="flex items-center gap-3">
                  <div className="flex -space-x-2">
                    {notice.recipients?.slice(0, 3).map((r, i) => (
                       <div key={i} className="size-7 rounded-full bg-white border-2 border-gray-100 flex items-center justify-center overflow-hidden">
                  ...
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    {notice.targetType === 'all' ? 'All Staff' : `${notice.recipients?.length || 0} Recipients`}
                  </span>
               </div>
            ...
            <div className="px-6 py-4 flex items-center justify-between text-[10px] font-bold text-gray-400">
               <div className="flex items-center gap-1.5">
                  <Calendar size={12} />
                  {notice.scheduledAt ? (
                    <span className="text-brand">Scheduled: {formatDate(notice.scheduledAt)}</span>
                  ) : (
                    <span>Created: {formatDate(notice.createdAt)}</span>
                  )}
               </div>
               {notice.isDraft && (
                 <span className="px-2 py-0.5 rounded-lg bg-gray-100 text-gray-500 uppercase">Draft</span>
               )}
            </div>
          </div>
        ))}
      ...
      {filteredNotices.length === 0 && (
        <div className="h-96 flex flex-col items-center justify-center bg-white rounded-[3rem] border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-500 max-w-sm mb-8 leading-relaxed font-medium">
            Keep your team informed by sending updates, policy changes, and important announcements.
          </p>
          <button
            onClick={() => setShowForm(true)}
            className="px-8 py-4 bg-brand text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-brand-800 transition-all shadow-lg shadow-brand/20"
          >
            Send First Notice
          </button>
        </div>
