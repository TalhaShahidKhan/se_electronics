import { paymentTypesEnum, productTypeEnum, createdFromTypesEnum, serviceTypeEnum, staffRoleEnum, subscriptionTypesEnum, serviceStatusEnum, statusTypesEnum } from "@/db/schema";
import { z } from "zod"
...
const BaseStaffDataSchema = z.object({
    ...
    nidFrontPhoto: z.file().min(1).mime(['image/jpeg', 'image/png', 'image/webp']),
...
export const StaffDataSchema = BaseStaffDataSchema
    .extend({
...
export const UpdateStaffDataSchema = BaseStaffDataSchema
    .extend({
    ...
    .transform(data => {
        ...
        return {
            ...
            ...(photo && mimeTypes.includes(photo.type) && photo.size > 0 && { photo: photo }),
            ...(nidFrontPhoto && mimeTypes.includes(nidFrontPhoto.type) && nidFrontPhoto.size > 0 && { nidFrontPhoto: nidFrontPhoto }),
            ...(nidBackPhoto && mimeTypes.includes(nidBackPhoto.type) && nidBackPhoto.size > 0 && { nidBackPhoto: nidBackPhoto }),
        }
    });
...
export const UpdateServiceDataSchema = ServiceDataSchema.extend({
    ...
    // serviceId: z.string(),
    serviceStatus: z.enum([...serviceStatusEnum.enumValues, 'custom', 'new_note']),
...
export const UpdateServiceInfoSchema = z.object({
    ...
    technicianComment: z.object({
        resolved: z.boolean(),
        explanation: z.string().optional(),
        travelCost: z.number().optional(),
        reason: z.string().optional(),
    }).optional(),
    status: z.enum(['pending', 'in_progress', 'appointment_retry', 'service_center', 'completed', 'canceled']).optional(),
    statusTrack: z.array(z.enum(['pending', 'in_progress', 'appointment_retry', 'service_center', 'completed', 'canceled'])).optional(),
    messageInfo: z.object({
        customerName: z.string().min(1),
...
export const PaymentDataSchema = z.union([WalletPaymentSchema, BankPaymentSchema]);
...
export const BaseSubscriptionDataSchema = z.object({
    ...
    ipsBrand: z.string().optional(),
    ipsPowerRating: z.string().optional(),
    paymentType: z.enum(paymentTypesEnum.enumValues),
})
